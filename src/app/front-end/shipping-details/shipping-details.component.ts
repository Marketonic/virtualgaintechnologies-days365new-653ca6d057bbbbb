...
export class ShippingDetailsComponent implements OnInit {
  ...
  get h() {
    return this.deliveryMethodForm.controls;
  }
  get i() {
  ...
  getCartItems() {
    ...
    this.homeservice.getCartItems(params).subscribe(
      (payload) => {
  ...
  inSubmit() {
    ...
    this.courierService.pincodeCheck(param).subscribe(
      (payload: any) => {
        var response = JSON.parse(JSON.stringify(payload));
        if (response.error) {
  ...
  async pay(cartitems) {
    ...
    await cartitems.forEach((element) => {
      element.cartData.productVariant.forEach((element2) => {
        if (element2._id === element.varientId) {
          console.log(element2);
          if (element2.stock < 1) {
            chekcstock = 1;
            console.log(element2.stock);
          }
        }
      });
    });
    if (chekcstock == 0) {
      ...
      this.paymentService.makePayment(params).subscribe(
        (payload: any) => {
          var response = JSON.parse(JSON.stringify(payload));
