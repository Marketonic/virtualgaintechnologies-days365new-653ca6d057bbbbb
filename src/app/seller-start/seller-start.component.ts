import { Component, OnInit, ViewChild, OnDestroy, HostListener, Input } from '@angular/core';
import { FormGroup,FormArray,ValidatorFn, FormControl, Validators, FormBuilder, Form, NgForm } from '@angular/forms';
...
export class SellerStartComponent implements OnInit, OnDestroy {
	...
	spinner = [false, false, false, false, false, false, false, false, false, false, false, false,false];
	errorMessage = ['', '', '', '', '', '', '', '', '', '', '', '',''];
	...
	constructor(
		...
		setTimeout(() => {
			...
			!this.multi && this.productCategoryForm.controls.orders.setValidators([
	...
	createForms() {
		...
		this.registerSellerForm = new FormGroup({
			...
			'shippingMethod': new FormControl('Fulfillment by Days365', [Validators.required])
		});


		let shippingFee: number = this.vendorData ? this.vendorData.shipping_fee : 0;
		let bankAccountDetails = (this.vendorData && this.vendorData.bank_account_details) ? this.vendorData.bank_account_details : null;
		let taxDetails = (this.vendorData && this.vendorData.tax_details) ? this.vendorData.tax_details : null;
		...
		this.shippingFeeForm = new FormGroup({
			'shippingFee': new FormControl(shippingFee, [Validators.required, Validators.min(0)])
		});

		this.bankAccountForm = new FormGroup({
			'accountHolderName': new FormControl(bankAccountDetails ? bankAccountDetails.account_holder_name : '', [Validators.required]),
			'accountType': new FormControl(bankAccountDetails ? bankAccountDetails.account_type : 'Savings Account', [Validators.required]),
			'accountNumber': new FormControl(bankAccountDetails ? bankAccountDetails.account_number : '', [Validators.required, Validators.pattern(/^[0-9]{9,18}$/)]),
			'ifscCode': new FormControl((bankAccountDetails && bankAccountDetails.IFSC_code) ? bankAccountDetails.IFSC_code : "", [Validators.pattern(/^[A-Z]{4}0[A-Z0-9]{6}$/)])
		});



		let tState = (!this.taxOption && taxDetails && taxDetails.state) ? taxDetails.state : 'Andhra Pradesh';
		let tSellerName = (!this.taxOption && taxDetails && taxDetails.seller_name) ? taxDetails.seller_name : '';
		let tGstNumber = (!this.taxOption && taxDetails && taxDetails.GST_number) ? taxDetails.GST_number : '';
		let tPanNumber = (!this.taxOption && taxDetails && taxDetails.PAN_number) ? taxDetails.PAN_number : '';
		this.taxDetailsForm2 = new FormGroup({
			...
			'gstNumber': new FormControl(tGstNumber, [Validators.required, Validators.pattern(/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/)]),
			'panNumber': new FormControl(tPanNumber, [Validators.required, Validators.pattern(/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/)])
		});

		let TaxCode = (this.vendorData && this.vendorData.product_tax_code) ? this.vendorData.product_tax_code : 'A_GEN_EXEMPT';
		let TaxCodePercentage = (this.vendorData && this.vendorData.taxCodePercentage) ? this.vendorData.taxCodePercentage : '0';
        let productTaxCode = TaxCode+'@'+TaxCodePercentage;
		this.isChecked[1] = (this.vendorData && this.vendorData.product_tax_code) ? true : false;
	...
	getVendorDetails() {
		this.vendorDetailsService.getVendorDetails()
	...
	createVendorRecord() {
		...
		this.spinner[0] = true;
		this.vendorDetailsService.createVendorRecord(this.companyNameForm.getRawValue())
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	isStoreNameAvailable(event) {
		...
		this.vendorDetailsService.isStoreNameAvailable(store)
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
				if (!response.error) {
	...
	updateSellerDetails() {
		...
		this.vendorDetailsService.updateSellerDetails(this.registerSellerForm.getRawValue())
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	routeTaxUpdate() {
		...
		switch (code) {
			case '0': this.updateTaxDetails(); break;
			case '1': this.updateGstExemptedStatus({ isGstExempted: true }); break;
			case '2': this.updateGstExemptedStatus({ isGstExempted: false }); break;
			default: return;
		}
	}
	...
	updateTaxDetails() {
		...
		this.spinner[2] = true;
		this.vendorDetailsService.updateTaxDetails(this.taxDetailsForm.getRawValue())
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	updateShippingFee() {
		...
		this.spinner[3] = true;
		this.vendorDetailsService.updateShippingFee(this.shippingFeeForm.getRawValue())
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	updateBankDetails() {
		...
		this.spinner[4] = true;
		this.vendorDetailsService.updateBankDetails(this.bankAccountForm.getRawValue())
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	updateGstExemptedStatus(status) {
		this.errorMessage[2] = '';
		this.spinner[2] = true;
		this.vendorDetailsService.updateGstExemptedStatus(status)
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
				this.spinner[2] = false;
	...
	updateDashBoardTaxDetails() {
		...
		this.spinner[5] = true;
		this.vendorDetailsService.updateTaxDetails(this.taxDetailsForm2.getRawValue())
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	updateProductCategory() {
		...
		this.vendorDetailsService.updateProductCategory(obj)
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	getStates() {
		this.stateList = [];
		this.vendorDetailsService.getStates({ country: "India" })
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
				if (!response.error) {
	...
	updateSignatureDraw() {
		...
		this.vendorDetailsService.updateSignature(formData)
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	getSignatureURL() {
		this.vendorDetailsService.getSignatureURL()
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
				if (!response.error) {
	...
	getFoodLicenseURL() {
		...
		this.vendorDetailsService.getVendorPrivateFileURL(params)
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
				if (!response.error) {
	...
	uploadFoodLicense() {
		...
		this.vendorDetailsService.uploadVendorPrivateFile(formData)
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
	...
	fileChange(event) {
		...
		if (fileList.length > 0) {
			...
			if (type != "image/jpeg" && type != "image/png") {
				...
				if (size > 500) {
					this.sizeExceed[0] = 'File too Big, please select a file less than 500KB.';
				}
				else {
	...
	fileChange2(event) {
		...
		if (fileList.length > 0) {
			...
			if (type != "image/jpeg" && type != "image/png") {
				...
				if (size > 500) {
					this.sizeExceed[1] = 'File too Big, please select a file less than 500KB.';
				}
				else {
	...
	fileChange3(event) {
		...
		if (fileList.length > 0) {
			...
			if (type != "image/jpeg" && type != "image/png") {
				...
				if (size > 500) {
					this.sizeExceed[2] = 'File too Big, please select a file less than 500KB.';
				}
				else {
	...
	fileChange4(event) {
		...
		if (fileList.length > 0) {
			...
			if (type != "image/jpeg" && type != "image/png") {
				...
				if (size > 500) {
					this.sizeExceed[3] = 'File too Big, please select a file less than 500KB.';
				}
				else {
	...
	fileChange5(event) {
		...
		if (fileList.length > 0) {
			...
			if (type != "image/jpeg" && type != "image/png") {
				...
				if (size > 500) {
					this.sizeExceed[4] = 'File too Big, please select a file less than 500KB.';
				}
				else {
	...
	getProductTaxCodes() {
		...
		this.vendorDetailsService.getProductTaxCodes()
			.pipe(takeUntil(this.destroyed$))
			.subscribe((payload) => {
				var response = JSON.parse(JSON.stringify(payload));
				if (!response.error) {
